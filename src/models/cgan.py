...
class Generator(nn.Module):
    ...
    def __init__(self, n_z, n_c, relu_slope=0.2):
        ...
        self.generator = nn.Sequential(
            nn.ConvTranspose2d(
                in_channels=1024 + 256, out_channels=512, kernel_size=4, stride=2, padding=1, bias=False,
            ),
            nn.BatchNorm2d(512),
        ...
        self.class_embedding = nn.Sequential(
            nn.ConvTranspose2d(in_channels=self.n_c, out_channels=256, kernel_size=4, stride=1, bias=False,
                              ),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(negative_slope=self.relu_slope, inplace=True),
        )
        self.feature_embedding = nn.Sequential(
            nn.ConvTranspose2d(in_channels=self.n_z, out_channels=1024, kernel_size=4, stride=1, bias=False,
                              ),
            nn.BatchNorm2d(1024),
            nn.LeakyReLU(negative_slope=self.relu_slope, inplace=True),
        )
    ...
    def forward(self, inputs, class_vals):
        return self.generator(
            torch.cat((self.feature_embedding(
